...
async def create_series_nfo(payload: SonarrWebhookSeriesAddPayload, tmdb: TmdbClient, tvdb: TvdbClient | None = None) -> None:
    ...
    title = (
        (isinstance(tvdb_payload, TvdbData) and tvdb_payload.name) or
        (tmdb_payload and tmdb_payload.name)
    )
    plot = (
        (isinstance(tvdb_payload, TvdbData) and tvdb_payload.overview) or
...
async def create_episode_nfo(payload: SonarrWebhookDownloadPayload, tmdb: TmdbClient, tvdb: TvdbClient | None = None) -> None:
    ...
    tvdb_payload = await tvdb.episodes_translations(payload.episodes[0].tvdbId) if tvdb else None
    ...
    title = (
        (isinstance(tvdb_payload, TvdbData) and tvdb_payload.name) or
        (episode_info and episode_info.name)
    )
    plot = (
        (isinstance(tvdb_payload, TvdbData) and tvdb_payload.overview) or
