...
async def get_mkv_info(episode_path: Path) -> None | dict[str, list[dict[str, Any]]]:
    ...
    while not await aio_os.path.exists(episode_path):
        if time.time() - start_time > 600:  # 超过10分钟仍未找到文件，退出循环
            logging.error("Timeout: File %s never appeared.", episode_path)
            return None
        logging.info("Waiting for %s to exist...", episode_path)
    ...
    while stable_checks < stable_checks_required:
        if time.time() - start_time > 600:  # 超过10分钟仍未找到文件，退出循环
            logging.error("Timeout: File %s never stabilized.", episode_path)
            return None
    ...
    if process.returncode != 0:
        logging.error("Error running mkvmerge -J on %s: %s", episode_path, stderr.decode().strip())
        return None
    ...
    try:
        ...
        logging.error("Failed to decode mkvmerge JSON output: %s", e)
        return None
...
async def mkv_merge(episode_path: Path) -> None:
    ...
    if mkv_info is None:
        logging.error("Failed to get MKV info for %s", episode_path)
        return
    ...
    if not subtitle_tracks or all(track.get("language") in ["chi", "zh"] or track.get("language_ietf") in ["zh-Hans", "zh-Hant"] for track in subtitle_tracks):
        logging.info("No subtitles found or only Chinese subtitles in %s, skipping merge", episode_path)
        return
    ...
    stdout, stderr = await process.communicate()
    if process.returncode != 0:
        logging.error("Error running mkvmerge on %s: %s", episode_path, stderr.decode().strip())
        return

    try:
        # output_path.rename(episode_path)  # 重命名输出文件为原始文件名
        await aio_os.rename(output_path, episode_path)
        logging.info("Successfully merged %s to %s", episode_path, output_path)
    except Exception as e:
        logging.error("Error renaming %s to %s: %s", output_path, episode_path, e)
