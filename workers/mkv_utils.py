...
async def get_mkv_info(episode_path: Path) -> None | dict[str, list[dict[str, Any]]]:
    ...
    while not await aio_os.path.exists(episode_path):
        if time.time() - start_time > 600:  # 超过10分钟仍未找到文件，退出循环
            logger.error("超时：文件 %s 从未出现。", episode_path)
            return None
        logger.info("正在等待 %s 存在...", episode_path)
    ...
    while stable_checks < stable_checks_required:
        if time.time() - start_time > 600:  # 超过10分钟仍未找到文件，退出循环
            logger.error("超时：文件 %s 从未稳定。", episode_path)
            return None
    ...
    if process.returncode != 0:
        logger.error("在 %s 上运行 mkvmerge -J 时出错：%s", episode_path, stderr.decode().strip())
        return None
    ...
    try:
        ...
        logger.error("无法解码 mkvmerge JSON 输出：%s", e)
        return None
...
async def mkv_merge(episode_path: Path) -> None:
    ...
    if mkv_info is None:
        logger.error("无法获取 %s 的 MKV 信息", episode_path)
        return
    ...
    if not subtitle_tracks or all(track.get("language") in ["chi", "zh"] or track.get("language_ietf") in ["zh-Hans", "zh-Hant"] for track in subtitle_tracks):
        logger.info("%s 中未找到字幕或只有中文字幕，正在跳过合并", episode_path)
        return
    ...
    stdout, stderr = await process.communicate()
    if process.returncode != 0:
        logger.error("在 %s 上运行 mkvmerge 时出错：%s", episode_path, stderr.decode().strip())
        return

    try:
        # output_path.rename(episode_path)  # 重命名输出文件为原始文件名
        await aio_os.rename(output_path, episode_path)
        logger.info("已成功将 %s 合并到 %s", episode_path, output_path)
    except Exception as e:
        logger.error("将 %s 重命名为 %s 时出错：%s", output_path, episode_path, e)
